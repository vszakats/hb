...
METHOD assign() CLASS Get
   ...
   IF ::hasFocus
      xValue := ::unTransform()
      IF ::cType == "C"
         xValue += hb_USubStr( ::original, hb_ULen( xValue ) + 1 )
      ENDIF
      ::varPut( xValue )
   ENDIF
...
METHOD display() CLASS Get
   ...
   IF ::cType == "N" .AND. ::hasFocus .AND. ! ::lMinusPrinted .AND. ;
      ...
      ::nPos > ::decPos .AND. Val( hb_ULeft( cBuffer, ::decPos - 1 ) ) == 0

      /* Display "-." only in case when value on the left side of
         the decimal point is equal 0 */
      cBuffer := hb_UStuff( cBuffer, ::decPos - 1, 2, "-." )
   ENDIF
   ...
   IF ! ::lSuppDisplay .OR. nDispPos != ::nOldPos
      ...
      hb_DispOutAt( ::nRow, ::nCol, ;
         iif( ::lHideInput, hb_UPadR( Replicate( hb_ULeft( ::cStyle, 1 ), hb_ULen( RTrim( cBuffer ) ) ), ::nDispLen ), hb_USubStr( cBuffer, nDispPos, ::nDispLen ) ), ;
...
METHOD end() CLASS Get
   ...
   IF ::hasFocus
      nLastCharPos := hb_ULen( RTrim( ::cBuffer ) ) + 1
      ...
      IF nLastCharPos > 2 .AND. ! ::IsEditable( nLastCharPos - 1 )
         FOR nFor := nLastCharPos - 2 TO ::FirstEditable() STEP -1
            IF ::IsEditable( nFor )
               IF Empty( hb_USubStr( ::cBuffer, nFor, 1 ) )
                  nLastCharPos := nFor
               ELSE
                  EXIT
               ENDIF
...
METHOD setFocus() CLASS Get
   ...
   IF ! ::hasFocus
      ...
      IF ::cType == "N"
         ::decPos := hb_UAt( iif( "E" $ ::cPicFunc, ",", "." ), ::cBuffer )
         IF ::decPos == 0
            ::decPos := hb_ULen( ::cBuffer ) + 1
         ENDIF
         ::lMinus2 := ( ::xVarGet < 0 )
      ELSE
      ...
      IF ::pos <= ::nMaxEdit
         ...
         ELSE
            ...
            IF ::nPos > ::nMaxEdit
               ::pos := ::FirstEditable()
            ENDIF
            ::cBuffer := hb_UStuff( ::cBuffer, ::nPos, 1, cChar )
      ...
      IF ::nPos <= ::nMaxEdit
         ...
         ELSE
            ...
            IF ::lPicComplex
               ...
               nMaxEdit := nFor
               ::cBuffer := ;
                  hb_ULeft( hb_UStuff( hb_ULeft( ::cBuffer, nMaxEdit - 2 ), ::nPos, 0, cChar ) + ;
                  hb_USubStr( ::cBuffer, nMaxEdit ), ::nMaxLen )
            ELSE
               ::cBuffer := ;
                  hb_ULeft( hb_UStuff( ::cBuffer, ::nPos, 0, cChar ), ::nMaxEdit )
            ENDIF
...
METHOD wordLeft() CLASS Get
   ...
   IF ::hasFocus
      ...
      ELSE
         ...
         nPos := iif( hb_USubStr( ::cBuffer, ::nPos, 1 ) == " ", ::nPos, ::nPos - 1 )
      ...
      IF cPicture != NIL
         ...
         IF HB_ISSTRING( cPicture )
            ...
            IF hb_LeftEq( cPicture, "@" )
               ...
               IF ( nAt := hb_UAt( " ", cPicture ) ) == 0
                  ::cPicFunc := hb_asciiUpper( cPicture )
                  ::cPicMask := ""
               ELSE
                  ::cPicFunc := hb_asciiUpper( hb_ULeft( cPicture, nAt - 1 ) )
               ...
               IF ( nAt := hb_UAt( "S", ::cPicFunc ) ) > 0
                  ...
                  IF Val( cNum ) > 0
                     ::nPicLen := Val( cNum )
                  ENDIF
                  ::cPicFunc := hb_ULeft( ::cPicFunc, nAt - 1 ) + hb_USubStr( ::cPicFunc, nFor )
               ENDIF
      ...
      FOR nFor := 1 TO ::nMaxLen
         ...
         IF cChar $ ",." .AND. hb_USubStr( cBuffer, nFor, 1 ) $ ",." // " " FIXME
            IF "E" $ cPicFunc
               cChar := iif( cChar == ",", ".", "," )
            ENDIF
            cBuffer := hb_UStuff( cBuffer, nFor, 1, cChar )
         ENDIF
      NEXT
   ...
   IF ::hasFocus
      ...
      IF HB_ISSTRING( cBuffer )
         ...
         CASE "C"
            ...
            IF "R" $ ::cPicFunc
               ...
               FOR nFor := 1 TO hb_ULen( ::cPicMask )
                  IF hb_asciiUpper( hb_USubStr( ::cPicMask, nFor, 1 ) ) $ "ANX9#!LY"
                     xValue += hb_USubStr( cBuffer, nFor, 1 )
                  ENDIF
               NEXT
            ELSE
               xValue := cBuffer
         ...
         CASE "N"
            ...
            lMinus := .F.
            IF "X" $ ::cPicFunc .AND. hb_URight( cBuffer, 2 ) == "DB"
               lMinus := .T.
            ENDIF
            IF ! lMinus
               FOR nFor := 1 TO ::nMaxLen
            ...
            ELSE
               IF "E" $ ::cPicFunc
                  cBuffer := ;
                     hb_ULeft( cBuffer, ::FirstEditable() - 1 ) + ;
                     hb_StrReplace( hb_USubStr( cBuffer, ::FirstEditable(), ::LastEditable() - ::FirstEditable() + 1 ), ".,", " ." ) + ;
                     hb_USubStr( cBuffer, ::LastEditable() + 1 )
               ELSE
                  cBuffer := ;
                     hb_ULeft( cBuffer, ::FirstEditable() - 1 ) + ;
                           StrTran( hb_USubStr( cBuffer, ::FirstEditable(), ::LastEditable() - ::FirstEditable() + 1 ), ",", " " ) + ;
                     hb_USubStr( cBuffer, ::LastEditable() + 1 )
               ENDIF
               ...
               FOR nFor := ::FirstEditable() TO ::LastEditable()
                  IF ::IsEditable( nFor )
                     IF lHasDec .AND. hb_USubStr( cBuffer, nFor, 1 ) == " "
                        cBuffer := hb_ULeft( cBuffer, nFor - 1 ) + "0" + hb_USubStr( cBuffer, nFor + 1 )
                     ENDIF
                  ELSE
                     IF hb_USubStr( cBuffer, nFor, 1 ) == "."
                        lHasDec := .T.
                     ELSE
                        cBuffer := hb_ULeft( cBuffer, nFor - 1 ) + Chr( 1 ) + hb_USubStr( cBuffer, nFor + 1 )
                     ENDIF
            ...
            IF lMinus
               FOR nFor := 1 TO hb_ULen( cBuffer )
                  IF IsDigit( hb_USubStr( cBuffer, nFor, 1 ) ) .OR. hb_USubStr( cBuffer, nFor, 1 ) == "."
                     EXIT
                  ENDIF
               NEXT
               nFor--
...
METHOD hitTest( nMRow, nMCol ) CLASS Get
   ...
   ELSE
      ...
      CASE nMRow == ::nCapRow .AND. ;
           nMCol >= ::nCapCol .AND. ;
           nMCol < ::nCapCol + hb_ULen( ::cCaption ) /* NOTE: C5.3 doesn't care about the shortcut key. */
         RETURN HTCAPTION
      ENDCASE
   ENDIF
...
METHOD PROCEDURE deleteLow() CLASS Get
   ...
   IF ::cType == "N" .AND. hb_USubStr( ::cBuffer, ::nPos, 1 ) $ "(-"
      ::lMinus2 := .F.
   ENDIF

   ::cBuffer := hb_UPadR( hb_UStuff( hb_UStuff( ::cBuffer, ::nPos, 1, "" ), nMaxLen, 0, " " ), ::nMaxLen )
...
METHOD Input( cChar ) CLASS Get
   ...
   IF ! Empty( ::cPicMask )
      cPic  := hb_asciiUpper( hb_USubStr( ::cPicMask, ::nPos, 1 ) )
