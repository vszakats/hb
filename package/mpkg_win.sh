...
(
  ...
  echo ---------------------------
  printenv | grep '_VER=' | grep -v '^_'
  echo ---------------------------
  printenv | grep -E '^(HB_USER_|HB_BUILD_|HB_WITH_|HBMK_WITH_|HB_STATIC_)' | grep -Ev '(HB_BUILD_POSTRUN_HOST=|HB_BUILD_POSTRUN=|HB_BUILD_PKG=)' | sed "s|${HOME}|~|g"
  echo ---------------------------
