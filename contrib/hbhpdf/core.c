...
{
   hb_retns( ( HB_SIZE ) HPDF_GetStreamSize( hb_HPDF_Doc_par( 1 ) ) );
}
...
HB_FUNC( HPDF_READFROMSTREAM )
{
   if( HB_ISBYREF( 2 ) )
   {
      ...
      if( size == 0 )
         size = HPDF_GetStreamSize( hb_HPDF_Doc_par( 1 ) );
      else if( size < 1024 )
         size = 1024;

      buffer = ( HPDF_BYTE * ) hb_xgrab( size + 1 );

      hb_retns( ( HB_SIZE ) HPDF_ReadFromStream( hb_HPDF_Doc_par( 1 ), buffer, &size ) );
   ...
   else
   {
      ...
      HPDF_ReadFromStream( hb_HPDF_Doc_par( 1 ), buffer, &size );

      hb_retclen_buffer( ( char * ) buffer, size );
   }
}
